...
public class TelephonePole : MonoBehaviour
{
  ...
  public TelephonePole NextPole
  {
    ...
    set
    {
      Undo.RecordObject(this, "Set NextPole");
      _nextPole = value;
  ...
  private void Awake()
  {
    ...
    bool includeOriginal = renderer.enabled;
    renderer.enabled = true;

    CombineInstance[] combine = new CombineInstance[includeOriginal ? 3 : 2];
    int index = 0;
    MeshFilter mf = GetComponent<MeshFilter>();
    if (includeOriginal)
    {
      Mesh original = mf.mesh;
      combine[index++].mesh = original;
    }
    combine[index++].mesh = meshL;
    combine[index++].mesh = meshR;

    for (int i = combine.Length - 2; i < combine.Length; i++)
